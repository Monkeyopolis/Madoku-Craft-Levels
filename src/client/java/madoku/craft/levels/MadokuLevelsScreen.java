...
public final class MadokuLevelsScreen extends Screen {
	...
	public MadokuLevelsScreen() {
		super(Component.literal("Madoku Levels"));
	}
	...
	protected void init() {
		this.addRenderableOnly((guiGraphics, mouseX, mouseY, partialTick) -> {
			...
			if (!snapshot.hasData()) {
				drawScaledCenteredText(guiGraphics, "Madoku Levels", this.width / 2, panelY + 18, HEADER_TEXT_SCALE, 0xFF404040);
				drawScaledCenteredText(guiGraphics, "Waiting for level data...", this.width / 2, panelY + 32, INFO_TEXT_SCALE, SUBTEXT_COLOR);
				return;
			}
